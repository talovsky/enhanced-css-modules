...
import { type AliasFromUserOptions, type PathAlias } from "./options";
...
function valueContainsWorkspaceFolder(value: string): boolean {
	return value.includes(WORKSPACE_FOLDER_VARIABLE);
}
...
function replaceWorkspaceFolderWithRootPath(pathAlias: PathAlias, rootPath: string): PathAlias {
	...
	for (const key in pathAlias) {
		...
		newAlias[key] = Array.isArray(aliasValue) ? aliasValue.map(replaceAlias) : replaceAlias(aliasValue);
	}
...
export async function getRealPathAlias(
	...
	if (workspaceFolder) {
		...
		const alias = { ...tsAlias, ...pathAliasOptions } as PathAlias;
